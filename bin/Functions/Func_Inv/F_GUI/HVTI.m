...
[ZOMBIE.PARA,ZOMBIE.MDL,ZOMBIE.HV,ZOMBIE.RPDC,ZOMBIE.RGDC,ZOMBIE.LPDC,ZOMBIE.LGDC,ZOMBIE.CONF,ZOMBIE.ON,ZOMBIE.ASV]=deal(false);
...
if exist('zombieRPDC.txt','file'),ZOMBIE.RPDC=true;end
if exist('zombieRGDC.txt','file'),ZOMBIE.RGDC=true;end
if exist('zombieLPDC.txt','file'),ZOMBIE.LPDC=true;end
if exist('zombieLGDC.txt','file'),ZOMBIE.LGDC=true;end
if exist('zombieMDL.txt','file'),ZOMBIE.MDL=true;end
if (ZOMBIE.HV||ZOMBIE.RPDC||ZOMBIE.RGDC||ZOMBIE.LPDC||ZOMBIE.LGDC)&&(ZOMBIE.PARA||ZOMBIE.MDL),
    ZOMBIE.ON=true;
...
if ZOMBIE.ON
    ...
    if (ZOMBIE.RPDC || ZOMBIE.RGDC || ZOMBIE.LPDC || ZOMBIE.LGDC) && ZOMBIE.HV
        % Only one DC available in the current version:
        fprintf(1,'Joint inversion of:\nHV\n');
        if ZOMBIE.RPDC
            fprintf(1,'Rayleigh Phase DC\n');
        elseif ZOMBIE.RPDC
            fprintf(1,'Rayleigh Group DC\n');
        elseif ZOMBIE.RPDC
            fprintf(1,'Love Phase DC\n');
        elseif ZOMBIE.RPDC
            fprintf(1,'Love Group DC\n');
        end
        THVDC_Callback([],[], handles);        
    elseif (ZOMBIE.RPDC || ZOMBIE.RGDC || ZOMBIE.LPDC || ZOMBIE.LGDC)
        % Only one DC available in the current version:
        fprintf(1,'Inversion of ');
        if ZOMBIE.RPDC
            fprintf(1,'Rayleigh Phase DC\n');
        elseif ZOMBIE.RPDC
            fprintf(1,'Rayleigh Group DC\n');
...
if ZOMBIE.ON
    ...
    if ~START_Callback([],[],handles)
        % No error in parameters / initial model
        smodel_Callback([],[],[]);
        if ZOMBIE.ASV
            MODELS;
        end
...
function [err_ID]=START_Callback(~, ~, handles)
[var,varoriginal,ZLVs,ZLVp,NC,HHS,ISDEPTH,ZOMBIE] =GetV ('var','var','ZLVs','ZLVp','NC','HHS','ISDEPTH','ZOMBIE');
err_ID=0;% Error type in parameters table: 
     % 0 = No fatal error (updating parameters can be required)
     % 1,2 = Vs or Vp need manual update (1 if problems with ZLV)
     % 3 = Severe error (e.g. Min>Max); 
     % 4 = Depths need manual update; 
     % 5 = Initial model out of range
     % 6 = Initial model doesn't fulfill LVZ or HHS conditions
     % 7 = Cancel was selected after updating parameters request
     % 8 = NaNs; % 9 = Poisson ratio outside physical range
...
if ~any(any(var(:,end-2:end-1)>=0.5)), % 0.5 not reached for poisson
    if ~any(any(isnan(var))),% No NaN value
        ...
        if any(var(:,5)<varoriginal(:,4))
            if ZOMBIE.ON
                fprintf(1,'Some minimum Vp values are larger than the maximum\n\n Please, change parameters\n');
            else
        ...
        if any(var(:,8)<varoriginal(:,7))
            if ZOMBIE.ON            
                fprintf(1,'Some minimum Vs values are larger than the maximum\n\n Please, change parameters\n');
            else
        ...
        if any(var(:,11)<varoriginal(:,10))
            if ZOMBIE.ON                        
                fprintf(1,'Some minimum density values are larger than the maximum\n\n Please, change parameters');
            else
        ...
        if any(var(:,14)<varoriginal(:,13))
            if ZOMBIE.ON
                fprintf(1,'Some minimum Poisson ratio values are larger than the maximum\n\n Please, change parameters\n');                
            else
                errordlg(sprintf('Some minimum Poisson ratio values are larger than the maximum\n\n Please, change parameters'),'Bad Condition');                
            end            
            err_ID=3;
        end
        if err_ID == 0; % Checks errors 1,2,4
            if ISDEPTH
...
%%%                end
        ...
        % Messages for errors 1,2,4
        if err_ID==4
            if ZOMBIE.ON
                fprintf(1,['Depth of bottom interface could not be generated for layer(s) ',cell2mat(cap4),' You are coding thickness instead of depth.\n']);
            else
        ...
        if ZLVp~=0
            if err_ID==1
                if ZOMBIE.ON                
                    fprintf(1,['Vs values could not be generated for layer(s) ',cell2mat(cap1),' Low velocity zones are forbidden.\n']);
                else
                    errordlg(['Vs values could not be generated for layer(s) ',cap1,' Low velocity zones are forbidden.']);                    
                end
            elseif err_ID==2
                if ZOMBIE.ON                                
                    fprintf(1,['Range of Vp, Vs and Poisson ratio are incompatible for layer(s) ',cell2mat(cap2),'\n']);
                else
        ...
        elseif ZLVp==0
            if err_ID==1
                if ZOMBIE.ON                                                
                    fprintf(1,['Vp values could not be generated for layer(s) ',cell2mat(cap1),' Low velocity zones are forbidden\n.']);
                else
...
%                 var(kkk)-varoriginal(kkk)
        ...
        [ini1]=GetV('ini1');% Check if initial models fulfills ranges and conditions
        if err_ID==0 && ~isempty(ini1)
            ...
            if ~ISDEPTH && any(var(:,1)>ini1(:,1)|var(:,2)<ini1(:,1))
                ...
                if ZOMBIE.ON
                    fprintf(1,['Thickness of initial model is out of range for layer(s) ',cell2mat(cap2),'\n']);
                else
            ...
            elseif ISDEPTH && any(var(1:end-1,1)>IniDepths | var(1:end-1,2)<IniDepths)                
                ...
                if ZOMBIE.ON                
                    fprintf(1,['Depth of initial model is out of range for layer(s) ',cell2mat(cap2),'\n']);
                else
            ...
            elseif any(var(:,4)>ini1(:,2)|var(:,5)<ini1(:,2))
                ...
                if ZOMBIE.ON                                
                    fprintf(1,['Vp of initial model is out of range for layer(s) ',cell2mat(cap2),'\n']);
                else
            ...
            elseif any(var(:,7)>ini1(:,3)|var(:,8)<ini1(:,3))
                ...
                if ZOMBIE.ON                                                
                    fprintf(1,['Vs of initial model is out of range for layer(s) ',cell2mat(cap2),'\n']);
                else
            ...
            elseif any(var(:,10)>ini1(:,4)|var(:,11)<ini1(:,4))
                ...
                if ZOMBIE.ON                                                                
                    fprintf(1,['Density of initial model is out of range for layer(s) ',cell2mat(cap2),'\n']);
                else
            ...
            elseif ZLVp==0 && any(diff(ini1(:,2))<0),
                if ZOMBIE.ON                                                                
                    fprintf(1,'Low velocity zones in Vp are forbidden.\n Incompatible initial model.\n');
                else
            ...
            elseif ZLVs==0 && any(diff(ini1(:,3))<0),
                if ZOMBIE.ON                                                                
                    fprintf(1,'Low velocity zones in Vs are forbidden.\n Incompatible initial model.\n');
                else
            ...
            elseif HHS~=0 && any(ini1(end,3)-ini1(:,3)<0),
                if ZOMBIE.ON                                                                
                    fprintf('Vs is not maximum for the halfspace in the initial model.\n Check model or settings.\n');
                else
                ...
                if any(var(:,13)>poi_ini1 | var(:,14)<poi_ini1)
                    ...
                    if ZOMBIE.ON                                                                
                        fprintf(1,['Poisson`s ratio of initial model is out of range for layer(s) ',cell2mat(cap2),'\n']);
                    else
    ...
    if ZOMBIE.ON                                                                    
        fprintf('The Poisson ratio values are larger than 0.5\n\n Please, change parameters\n');
    else
